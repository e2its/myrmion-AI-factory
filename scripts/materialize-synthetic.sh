...
[ "$RC" -eq 1 ] && printf '%s' "$OUT" | grep -q 'FEAT-001/TC-02' && printf '%s' "$OUT" | grep -q 'no linking test' && ok "RED: a declared case with no linking test at the one home (pytest-marker, from the materialised config)" || bad "unlinked case not refused (rc=$RC)" "$OUT"
OUT=$(cd "$P" && python3 scripts/gate.py traceability --baseline --init 2>&1 && python3 scripts/gate.py traceability 2>&1); RC=$?
[ "$RC" -eq 0 ] && printf '%s' "$OUT" | grep -q '1 linked, 1 unlinked (1 in the baseline)' && ok "the baseline records the debt once; the gate is green with the number on the board" || bad "baseline init/check wrong (rc=$RC)" "$OUT"
...
[ "$OUT" = "FEAT-001/TC-01=linked,FEAT-001/TC-02=linked" ] && ok "QA reads every case's status and tests from the gate's JSON — never re-derived" || bad "traceability json wrong" "$OUT"
