...
| sub-increment push (to its train) | **light** profile: every member that needs no build and no database — ADR sync, retired terms, budgets, law parity, currency, manifest parity, surface, runtime-surface parity, agents, the seal's state, the planning artefacts' governance digests, the test-case traceability (every declared case linked at its one home, the baseline shrinking) (and, in the framework repo, manifest drift validation and applicability) — all report, one verdict; secrets per pushed ref; the review and coherence markers | **full** profile | writes none |
...
| pull request to the main branch (CI) | **full** (the workflow runs `gate.py profile --run`; a sub-increment PR into its train owes light) | full | honours the loop's seal (`seal` member) |
