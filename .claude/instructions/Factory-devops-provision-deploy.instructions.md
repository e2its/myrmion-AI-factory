...
BOTH (EVOL-054): the protected branch is defended on the server — `python3 scripts/gate.py scm-protection --control-point ci` with the project's read-only token (RED = apply docs/scm/protection.md before provisioning production; n/a without a token = the runbook's checklist ticked by an administrator is the record)
```
