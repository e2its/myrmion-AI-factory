...
TIER_2_INFRASTRUCTURE:
  ...
  questions: [Q15, Q16, Q17, Q18, Q19, Q20, Q20.1, Q20.2, Q21, Q21.1, Q21.2, Q22, Q22.1, Q23, Q23.1, Q23.2, Q24, Q25, Q26, Q27, Q27.1, Q27.2, Q27.3, Q27.4, Q27.5, Q27.6, Q28, Q28.1, Q29, Q29.1, Q30, Q31, Q32, Q33, Q34]
...
#### Q21: CI/CD Platform
- **Options:** GitHub Actions | GitLab CI | Bitbucket Pipelines | Azure DevOps | Jenkins | CircleCI | AWS CodePipeline | GCP Cloud Build | None (the literal is the one the workflow templates' `stack_conditional` names — `ci_cd_platform == "Azure DevOps"`)
- **Persist:** `ci_cd.platform`

#### Q21.2: SCM host and its branch protection (EVOL-054)
- **Type:** the platform that hosts the repository, asked apart from the CI platform (they differ more often than not: Bitbucket + Jenkins, GitLab + GitHub Actions never, Azure Repos + Azure DevOps pipelines usually).
...
- **Type:** Single-select
- **Options:** `vision` (preview cards are cut from the vision component library only) | `code-manual` (a project tool renders the real components into a cards folder inside the repo and the operator runs it — usually a design-system-from-code skill they ask Claude to run; ask ONE follow-up: the folder) | `code-rebuild` (same, and the tool also has a terminal command; ask TWO follow-ups: the folder, the one command that rebuilds it — the only case the builder's `--rebuild` and the CI workflow can run) | `defer` (decide later — keys stay `null`, enabled afterwards by editing `subproducts/po-package/po-package.config.json`)
...
#### Q32: Delivery Mode (EVOL-046)
- **Type:** Single choice — closed set
- **Options:** `development` | `production`
- **Simplified:** es: "¿El proyecto está en construcción o ya en producción? En construcción, un sub-incremento hacia su tren pasa solo las puertas que no necesitan build ni base de datos; todo lo que llega a la rama principal pasa siempre todas." / en: "Is the project under construction or already in production? Under construction, a sub-increment pushed to its train owes only the gates that need no build and no database; everything that reaches the main branch always owes them all."
