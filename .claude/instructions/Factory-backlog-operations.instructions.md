...
>
> **Blocked-by label creation.** The `blocked-by:#{N}` label is NOT pre-created at `--init-board` because the value `#{N}` is per-issue. Instead, BACKLOG creates the label on-demand the first time it is applied (via `add_label`), or adapter-specific equivalents (GitHub creates labels on first use via `--force`; Jira and Linear require explicit creation — handled inside the adapter). The label persists after creation for reuse.
>
