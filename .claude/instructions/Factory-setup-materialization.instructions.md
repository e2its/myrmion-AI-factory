...
- Resolve the two surface placeholders against Q31 (EVOL-045) — bare integers replacing unquoted tokens: `{{SURFACE_CEILING_FILES}}` ← `surface.ceiling_files` · `{{SURFACE_CEILING_LINES}}` ← `surface.ceiling_lines`. `surface.escapes` (the closed vocabulary) and `surface.runtime_surface` (`[]` = every train deploys) keep template defaults. Read by `python3 scripts/gate.py surface` (pre-push, preflight Block 21, CI) and by BLUEPRINT's surface estimate.
