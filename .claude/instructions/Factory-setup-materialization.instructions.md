...
- Resolve the surface placeholders — bare tokens replaced by JSON values: `{{SURFACE_CEILING_FILES}}` ← `surface.ceiling_files` · `{{SURFACE_CEILING_LINES}}` ← `surface.ceiling_lines` (Q31, integers) · `{{RUNTIME_SURFACE}}` ← `surface.runtime_surface` (Q33, a JSON array of glob strings — the positive list every deploying workflow asks through `gate.py runtime-surface --changed`, EVOL-047). `surface.escapes` (the closed vocabulary) and `surface.declared_reads` keep template defaults. Two more bare tokens resolve from Q21 (`ci_cd.platform`) — the CI platform's own workflow files, one definition for the hard exclusion and for the parity scan:
  ...
  Add the project's own deploy workflows to `deploying_workflows` when DEVOPS creates them. `{{VERIFICATION_GATES}}` (EVOL-051) ← the path-to-gate map, a JSON object derived from the stack and Q33: every full-loop gate the BVL resolves for this stack (`tests`, `coverage`, `lint`, `typecheck`, `build`, `format`, `sast`, `complexity`, `seed-alignment` — only those with a command) with `reads` = the runtime surface ∪ the test roots (`seed-alignment` reads the migration and seed paths) and `command` = the BVL command (`tests` and `coverage` share the suite command: one execution feeds both). `documentation.paths` / `exclusions` land as the template defaults (`**/*.md`, `docs/**`, `.gitignore`; the workflows and the behavioural contracts excluded) — the ONE definition the preflight, the planning gate and the seal read. `{{TRACEABILITY_HOME}}` (EVOL-053) ← the one home of the case → test link, a JSON object derived from `backend.runtime` (and the frontend runtime when the backend has none): Python → `{"kind": "pytest-marker"}` and the template `tests/conftest_traceability.py` lands (stack-conditional); the materialisation then WRITES the wiring, never leaves it to prose — flat layout: `tests/conftest.py` with `pytest_plugins = ["conftest_traceability"]`; package layout (`tests/__init__.py`): the root `conftest.py` with `pytest_plugins = ["tests.conftest_traceability"]` — and `addopts = --strict-markers`, `testpaths = tests` in the pytest config (the plugin refuses to run without strict markers). Node.js / TypeScript / Go → `{"kind": "title-tag"}` (a literal `[FEATURE/CASE]` opening the title of `test(` / `it(` / `t.Run(`; a `describe`, a skipped or todo test, or a `fmt.Sprintf` title links nothing). Java / Kotlin (JUnit 5) → `{"kind": "junit-displayname"}` (`@DisplayName("[FEATURE/CASE] …")`). Rust and any other runner → `{"kind": "custom", "pattern": "<regex with a named group id, matched per line>"}` chosen by RDR, or `required: false` with its reason as a recorded decision. `traceability.test_roots` is derived with the home — Python `["tests/**"]`; Node.js / TypeScript `["tests/**", "e2e/**", "src/**/*.test.*", "src/**/*.spec.*"]`; Go `["**/*_test.go"]`; Java / Kotlin `["src/test/**"]`; Rust `["src/**", "tests/**"]`. After the tree lands, when plans already exist (a brownfield adoption): link at least one case at the home, then `python3 scripts/gate.py traceability --baseline --init` once (it refuses when nothing links — a home or a root that matches nothing is a misconfiguration, not debt) and commit the baseline so the gate ships green with the debt on the board. `{{SCM_PLATFORM}}` (quoted, EVOL-054) ← `scm.platform` (Q21.2) · `{{SCM_REQUIRED_CHECKS}}` ← `scm.required_checks` (a JSON list: the governance check's name on the chosen CI platform, e.g. `["governance-check"]`, plus the lock-step check where the project runs one) · `{{SCM_APPROVALS}}` ← `scm.approvals` (an integer, the project's decision). The runbook `docs/scm/protection.md` lands from `.context/templates/setup/scm/protection.<platform>.md` (stack-conditional on `scm.platform`: github, gitlab, bitbucket, azure-devops, other — exactly one) with the same three placeholders resolved. The materialisation never writes the protection itself (that needs an administrator's token and belongs to the repository's administrators): it delivers the runbook, the config and the reader (`python3 scripts/gate.py scm-protection`, a profile member at the `ci` control point); the report block (`### Server-side protection — next steps`) says whether CI carries a read-only token or the checklist is the record. The workflow templates already carry the wiring where the runner owns a token — GitHub Actions: `env: GH_TOKEN: ${{ github.token }}` on the profile step (metadata read); Azure Pipelines: `AZURE_DEVOPS_TOKEN: $(System.AccessToken)` — and name the variable to set where it does not (GitLab: `GITLAB_TOKEN`, scope `read_api`, masked; Bitbucket: `BITBUCKET_TOKEN` as `user:app_password`, secured); the materialisation never edits that wiring out. `{{AGENT_WRITER_MODEL}}` ← `agents.writer_model` · `{{AGENT_CRITIC_MODEL}}` ← `agents.critic_model` (Q34, EVOL-049 — quoted tokens, harness aliases; they must differ). **Invariant:** `python3 scripts/gate.py agents` is green on the materialised tree (the roster under `.claude/agents/` landed with its manifest entries, the tool matrix per class holds, the two families differ), else BLOCK. The `planning` block (EVOL-048): `{{PLANNING_GOVERNED_PATHS}}` ← the Q33 `surface.runtime_surface` list **plus** the governance paths `[".claude/**", ".github/workflows/**", "config/**", "docs/constitution.md", "docs/setup.md"]` (a JSON array — the governed paths are the runtime code and the governance inputs, seeded from the same answer as the deployment trigger); the documentation exemption, the gate-input carve-out, the exempt branch classes, the adoption window and the approval TTL keep template defaults. Its hooks (`check-plan-approval.sh`, `record-plan-approval.sh`, `check-plan-mode.sh`) land with `settings.json` — the block and the wiring land in the same step, never one without the other (a wired hook with no `planning` key blocks every write except the one that adds the key). **Invariant:** `python3 scripts/gate.py runtime-surface` is green on the materialised tree (the parity gate: at least one deploying workflow found; every path it or its scripts read is on the list, a hard exclusion or a declared read), else BLOCK. Read by `python3 scripts/gate.py surface` (pre-push, preflight Block 21, CI) and by BLUEPRINT's surface estimate.
...
3. `.context/templates/setup/claude/hooks/*.sh` → `.claude/hooks/*.sh`
   ...
   - `--upgrade`: overwrite each script with the template version. These scripts are framework-owned primitives — the target version is authoritative. No user customisation expected; project-specific branch protocol lives in constitution + ADRs, not in hook scripts. Current chain (PreToolUse): `check-branch-protection.sh`, `check-plan-approval.sh` (EVOL-048), `check-concurrency-lock.sh`, `check-governance-drift.sh`, `deliver-governance.sh`, `check-completion-gate.sh`, `check-ipp-compliance.sh`, `check-plan-mode.sh` (EVOL-048, matcher `EnterPlanMode`), `check-push-preflight.sh`; PostToolUse: `record-plan-approval.sh` (matcher `ExitPlanMode`, EVOL-048 — the only writer of the plan-approval marker). The first five match `Edit|Write`; `check-completion-gate.sh` and `check-ipp-compliance.sh` match `Write` only; `check-push-preflight.sh` matches `Bash` only (Factory PR Review push gate — invokes the factory-pr-review skill's `scripts/preflight.sh` when the Bash command is `git push`).
