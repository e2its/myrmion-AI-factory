...
3. **Regenerate governance snapshot** (`.context/governance_snapshot.md`):
   ...
   - The new format (EVOL-026+) embeds operational law verbatim: `## [LAW]` sections of constitution + universal DCs (`applicable_when: always`). If the upgraded constitution lacks `[LAW]` markers (legacy v < 3.0.0), the upgrade MUST add them per the materialization template before regen, otherwise the snapshot's `## Active Constitution` section will be empty and CI will fail
