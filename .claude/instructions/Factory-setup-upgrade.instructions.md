...
**Step 8 — Update Snapshot:** Record new checksum in project `governance_versions.json`. When the project manifest carries no `delivery_mode` (a project materialised before EVOL-046), ask Q32 (Factory-setup-discovery) and write the answer at the top level — an absent key already resolves to `production` (every push owes the full profile), so say it rather than leave it implicit. When the project `config/quality.json` carries no `agents.families` (a project materialised before EVOL-049), ask Q34 (writer / critic model aliases — two different families) and materialise `.claude/rules/agents.md` + `.claude/agents/*.md` from the template tree. When `config/quality.json` carries `planning.docs_exempt` (a project materialised before EVOL-051): move that list to `documentation.paths`, add `documentation.exclusions` (the template defaults: the workflows and `.claude/{instructions,skills,commands,hooks}/**`) and the `verification` block (`seal.required: true`, `gates` derived from the stack and Q33 exactly as materialization resolves `{{VERIFICATION_GATES}}` — never a literal placeholder, `digest_artefacts: ["design.md"]`), in the same step as the `scripts/gates/` refresh (`seal.py`, `digests.py`, `traceability.py` arrive with it); until then the planning gate refuses the retired key and every governed write is blocked. When it carries no `traceability` block (pre-EVOL-053): add it (the home derived from the stack exactly as materialization resolves `{{TRACEABILITY_HOME}}`, the template's grammar, plans, roots and baseline path), deliver `tests/conftest_traceability.py` for a Python stack, then record the debt once — `python3 scripts/gate.py traceability --baseline --init` — so the gate ships green with the unlinked cases on the board, never red on half the suite. When it carries no `scm` block (pre-EVOL-054): ask Q21.2 (the SCM host, the approvals, the required checks), add the block and deliver `docs/scm/protection.md` from the platform's template; the repository's administrators apply it, CI verifies it (`gate.py scm-protection` at the `ci` control point) when it carries a read-only token.
