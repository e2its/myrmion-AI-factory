...
Agentic code review engine. 6 vendored review agents + orchestrating SKILL. Single engine, two invocations: IMPLEMENT increment mode (scope=increment — the IMPLEMENT phase agent's per-increment review) and factory-pr-review Block 20 push gate (scope=branch, writes the proof-of-execution marker). Introduced by EVOL-039 (ADR-EVOL-039); since EVOL-049 the engine is the correctness lens's instrument (`factory-critic-correctness`, class `work-critic`).
...
All six (EVOL-049, frontmatter — recorded here, re-sync must re-apply): `tools: Read, Grep, Glob` + `class: work-critic` + `lens: correctness`; no `model` key.
...
- `agents/*.md` — the 6 vendored review agents (frontmatter `tools` + `class: work-critic` + `lens: correctness`; the model is resolved per spawn by `gate.py agents --resolve`, never carried in the file).
