...
> **Scope note.** This skill is the **push gate**. It is NOT the inline per-increment review that runs during `IMPLEMENT --build` — the IMPLEMENT phase agent's increment mode, where the read-only work critics (`factory-critic-*`, class `work-critic`) are spawned on each completed increment; the loop's cap and terminal are `rules/agents.md → agents.rounds` (review-policy.md carries the lenses' checklists, not the loop) and it lives inside the BVL cycle. Since EVOL-039 the two share ONE code-quality engine — `factory-code-review`, the correctness lens's instrument — invoked twice: the increment mode runs it per increment (findings feed `peer_review_{INC-N}_*.md`), the push gate spawns it on the cumulative branch diff as the correctness lens (writes the Block 20 marker). The push gate consumes ONLY the marker, never the peer_review files — file-coverage union over per-increment reviews is a false green (a file touched by two increments accumulates partial reviews, none over its final state). The 14 framework-specific checks (correctness + governance lenses) and the review-policy retry/override ladder remain increment-mode concerns.
...
- `.claude/rules/review-policy.md` — BVL/IMPLEMENT increment-mode review policy (worker ↔ critic retry loop, escalation, override, environment-driven strictness). Different reviewer, different lifecycle. The generic code-quality lens both reviewers need is shared via `factory-code-review` (single engine); review-policy.md continues to govern only the increment loop's mechanics.
- `.claude/rules/security_policy.md` — high-level project security policy (allowed crypto, PII handling, OWASP stance, threat model). Consumed by the security lens (`factory-critic-security`) in IMPLEMENT --build (SAST + dependency audit), QA --verify (DAST), and AUDIT (compliance dimension). The push gate's secret-pattern regexes are NOT here — they live in `scripts/detect_change_type.py` and may be extended only via skill changes (manifest-tracked), not via project rules.
...
- `.claude/rules/architecture.md` — layer dependency rules, module boundaries. Consumed by BLUEPRINT (design.md §7.8 Mandatory Patterns), the work critics in IMPLEMENT increment mode (Check #1), AUDIT. The push gate does NOT enforce architecture; that is a deeper, design-time concern.
