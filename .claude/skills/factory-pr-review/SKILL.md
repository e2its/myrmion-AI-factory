...
After every review cycle (preflight blocker resolution, manual `--review`, `/loop pr review` iteration), the agent MUST post a structured comment to the PR that captures the full analysis + fix trail. Chat sessions disappear; the PR is the durable audit record.

### When to post

- `--preflight` mode that produced blockers AND the author fixed them: post once after the fix lands (single comment per fix-cycle, not per finding).
- `--review {PR}` mode: post once per review run.
- `/loop pr review` iteration: post once per iteration that produced findings + fixes; suppress on iterations with zero findings (no value in noise).
- Phase 0 Coherence Audit: each marker emission ALSO surfaces in the PR comment when blockers are surfaced or detector results are non-trivial.

### What to include

The comment MUST contain:

1. **Exit state table** — every gate state at the iteration end. Minimum rows: `mergeStateStatus`, CI workflow status (per check), `preflight.sh` findings count, Phase 0 detector summary, governance-script local results (`validate-governance.sh`, `check-applicability-frontmatter.sh`, `check-iteration-id-format.sh`, `check-adr-constitution-sync.sh`, and any project-specific gates), Phase 0 marker file path.
2. **Per-iteration root-cause block** — for each iteration: symptom (what failed and where), root cause (the underlying bug, not just the surface error), fix (commit SHA + one-line description + manifest bump if any), verification (which local check + which CI run confirms green).
3. **Phase 0 detectors table** — explicit per-detector result (13 Reference coherence / 14 Placeholder leakage / 15 Manifest↔disk / 16-meta or 16-downstream Lock-step / 17 Commit↔diff / 18 Bump severity). Mark false-positive questions with `❓` + brief justification.
...
5. **Stale-signal disclaimer** — if earlier `github-actions[bot]` failure comments exist on the PR, explicitly call them out as historical so reviewers don't act on them.

### Format

- Markdown, top-level heading `## 🔁 PR-Review Loop — Iteration Log` (or `## 🔍 PR-Review — Manual Audit` for `--review` mode).
...
This contract is **universal** — it applies in both `meta` and `downstream` contexts. Downstream PRs whose review cycle resolves findings benefit from the same audit trail. The only carve-out: if the PR consists of zero-finding iterations only (the author pushed clean and CI passed first try), no comment is required.

### Why

Without persisting the analysis on the PR, the chain "I saw a failure → I diagnosed → I fixed → I verified" lives only in the chat transcript. Reviewers who arrive later have no way to reconstruct the loop's reasoning, and the same root cause can re-emerge in future PRs (no institutional memory). The PR comment is the framework's institutional memory for review-cycle work.
