...
if [ -f "docs/rules/ux-constitution.md" ]; then
  # Try to extract enforcement level (simplified grep)
  EXTRACTED=$(grep -oP 'BRAND_ENFORCEMENT_LEVEL.*?:\s*\K(BLOCKER|WARNING|MIXED)' docs/rules/ux-constitution.md 2>/dev/null || echo "WARNING")
