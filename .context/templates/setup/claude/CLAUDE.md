...
The banner is produced deterministically by `scripts/validate-governance.sh --banner` wired as a `SessionStart` hook. If it does not appear, governance is not loaded — investigate before proceeding. If the snapshot is missing or the hashes diverge from `docs/constitution.md` + `docs/setup.md`, the `UserPromptSubmit` freshness gate (`scripts/governance-onprompt.sh`) blocks the prompt with a `Governance snapshot stale` message; the resolution path is `/setup --upgrade`. See [Factory-governance-loading/SKILL.md](.claude/skills/Factory-governance-loading/SKILL.md) for the 3-tier design (banner / freshness gate / post-compact re-inject).
...
1. **Constitutional Supremacy + ADR Binding**: `docs/constitution.md` is LAW (except during SETUP/AUDIT). `.claude/rules/` holds detailed default regulations. **ADRs** (`docs/project_log/adr/*.md` for project-wide decisions + `docs/spec/{ID}/adr/*.md` for feature-scoped ones) hold **binding architectural decisions** — they override `.claude/rules/` defaults and are as enforceable as the constitution within the scope they cover. Hierarchy on conflict: `constitution.md` > `ADR` > `.claude/rules/`. Modifying an ADR without a superseding ADR is a governance-scope violation. Enforcement is already wired end-to-end — BLUEPRINT design.md §7.8 (Mandatory Architectural Patterns + ADR Bindings), GCRP RED_ZONE gate, and IMPLEMENT Review Check #14 (`[DESIGN-ADR]`) — this section exists so the agent knows ADRs are first-class governance even when operating outside slash commands.
