...
**Artefact.** `docs/spec/{FEATURE_ID}/increment_plan.md` — sidecar of `design.md`. `§ 0` Slicing Rationale, `§ 1` Increments (each declaring `Status`, `scenarios_covered`, `contract_surface`, `depends_on` (canonical DAG), `deployable: production`, acceptance checklist, branch name), `§ 2` Monolithic Escape Declaration (only when monolithic), `§ 3` Human-readable Dependency Diagram (Mermaid, non-authoritative — derived from § 1 `depends_on:`). Generated at `BLUEPRINT --start` via RDR (≥3 slicing alternatives, verbatim ratification).
