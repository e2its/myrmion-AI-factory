...
1. **Constitutional Supremacy (single source of truth)**: `docs/constitution.md` is the SOLE source of operational law (except during SETUP/AUDIT). The active law lives ONLY there — agents read it via the governance snapshot which embeds the `## [LAW]` sections verbatim. `.claude/rules/` holds detailed default regulations consumed on-demand for specific compliance checks; among them, `defect-prevention.md` universal entries (`applicable_when: always`) also embed in the snapshot. **ADRs** (`docs/project_log/adr/*.md`) are **historical records** of why a constitutional change was made — they hold context, alternatives considered, and consequences, but they are NOT active law. An ADR transitioning to `status: accepted` MUST amend `docs/constitution.md` in the same PR through the `Factory-adr-management` Accept Procedure, which mechanically copies the ADR's `## Operational Rule` field into a new or existing `## [LAW]` section. CI gate `scripts/check-adr-constitution-sync.sh` blocks accept-without-amendment PRs (bypass via `[adr-backfill]` commit marker for one-shot historical migration). **FDRs** (Feature Decision Records, `docs/spec/{ID}/fdr/*.md`) are feature-scoped binding decisions that stay local to their feature spec — they do NOT amend the universal constitution. Hierarchy on conflict: `constitution.md` > `FDR` (within feature scope) > `.claude/rules/`. Modifying constitution operational law without an ADR ceremony is a governance-scope violation.
