...
## Inputs
The libraries and services in scope with the **pinned version** of each (the stack of `docs/setup.md`; the dependency manifests the work touches — package.json, pyproject.toml, uv.lock, requirements*.txt, go.mod, Cargo.toml, pom.xml, build.gradle*, infra/**) and the questions the work rests on. No corpus digest: you read documentation, not the corpus.
