...
FUNCTION render_next_steps(actions, FEATURE_ID):
  ...
  phase = derive_current_phase(state)  # Uses factory.agent.md PHASE_MAP
  phase_label = HUMANIZE_PHASE(phase)  # From factory.agent.md helper
  ...
  progress_bar = render_progress_bar(progress_pct)  # From factory.agent.md helper

  # Humanized explanations for each action
  level = session.explanation_level OR "SIMPLIFIED"
  lang = session.language OR "en"
  FOR EACH action IN actions:
    action.user_explanation = HUMANIZE_ACTION(action, lang)  # From factory.agent.md helper
