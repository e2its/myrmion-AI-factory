...
Feedback report from a downstream materialised project lists seven gaps observed in their materialised state. Triage in the framework meta repo confirmed five as real upstream gaps; two were declassified.
...
This explains the gap pattern in the reporting project — content gaps in agent-consumed templates plus a missing inventory-drift signal plus pseudocode-only governance script plus widespread broken instruction references. None of them propagate today even when fixed.

Five gaps confirmed in framework upstream (not just materialised downstream):

1. `.context/templates/codesign/user_journey_template.md` ships a master-table format for journey steps. Downstream parsers require parser-canonical step blocks (`### Paso N` + per-step `### Schema:` / `DataIn:` / `DataOut:` markers) for deterministic extraction. Current template forces every materialised project to patch locally.
...
Two reported gaps declassified during triage:

- "SessionStart loads ~90 KB full snapshot, harness truncates" — framework SessionStart hook is `validate-governance.sh --banner` (one line). Snapshot is loaded on demand by `Factory-governance-loading/SKILL.md` Step 0, not by SessionStart. The truncation observed in the reference implementation is a project-local SessionStart customisation; redirected to the reference implementation.
...
- **Slug:** A — `feature/EVOL-027-downstream-governance-completeness`. Captures the architectural objective (close the propagation cycle) without binding to the reporting project as origin (B) or to gap #5 alone (C).
...
- Reported gap #3 (full snapshot at SessionStart truncated by harness) — declassified as project-local SessionStart customisation, redirected to the reporting project.
- Reported gap #5 (canonical pre-flight pattern) — declassified as cross-cutting protocol concern; deserves its own EVOL.
...
- **Fase 5 first (reformat user_journey) without fase 1 manifest extension.** Rejected — fix would die in upstream; the reporting project would still need a manual local patch. Defeats the unifying objective.
...
- **The reporting project specifically:** receives gap #1 (parser-canonical journey template), gap #2 (real snapshot script), gap #3 (instruction-ref sweep), gap #4 (drift detection), gap #5 (manifest extension) on next `SETUP --upgrade` after this EVOL ships.
...
- Triggered by: a downstream project's feedback report listing 7 gaps; framework triage on 2026-05-05 confirmed 5 as upstream gaps and declassified 2.
