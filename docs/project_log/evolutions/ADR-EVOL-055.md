...
`check-lockstep-pairs` (the `test_gates.py` twin), `validate-governance --base origin/main`, `manifest-parity`, `retired-terms`, ADR sync both directions; `test-gates.sh`, `test-code-review-gate.sh` (the review script), `materialize-synthetic.sh` (the adapters and the sync script land), `test-templates-static.sh`; `grep -rIinE "\bmass\b|nexus|tech-link" .claude .context scripts config CLAUDE.md README.md docs` — case-insensitive, minus the English uses of the word (mass-rename, mass sed sweep, mass-registration, mass assignment, mass delete) — → 0 lines on the two branches combined (the first sweep was case-sensitive and missed a lowercase example key and an uppercase tracker name; the critic found them).
